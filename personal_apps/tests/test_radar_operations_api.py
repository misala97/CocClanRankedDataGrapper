...
PLAIN_USERNAME = 'pytest radar ops nonadmin'
...
def plain_user():
    ...
    The name is unique in the schema, so the row is cleared BEFORE the insert
    too: a run killed between creating this account and dropping it would
    otherwise fail every later run on the constraint.
    """
    from werkzeug.security import generate_password_hash

    with flask_app.app_context():
        AppUser.query.filter_by(username=PLAIN_USERNAME).delete(
            synchronize_session=False)
        db.session.commit()
        user = AppUser(username=PLAIN_USERNAME,
...
def test_a_counter_a_run_never_measured_stays_null(app_context, owned_runs):
    """Counters may be added without bumping the schema version, so an older
    run genuinely never measured a newer one. Reporting 0 for it would be the
    exact lie this endpoint exists to avoid -- and only THAT counter goes
    null; the ones every run did report still add up."""
    partial = _counts(posts_seen=4)
    del partial['mentions']
    _run(owned_runs, QUIET, status='ok', summary=partial)
    _run(owned_runs, QUIET.replace(hour=13), status='ok',
         summary=_counts(posts_seen=6, mentions=2))

    day = activity.summary(QUIET, 1)['days'][0]
    assert day['posts_seen'] == 10
    assert day['mentions'] is None
    assert day['counted_runs'] == 2


def test_runs_written_in_another_schema_version_are_not_added_in_silence(
        app_context, owned_runs):
    """The version exists because a counter's MEANING changed, so mixing them
...
def test_summary_refuses_an_unsupported_window_itself(app_context):
    """The route validates first, but the function does not trust its caller."""
    with pytest.raises(ValueError):
        activity.summary(QUIET, 2)
...
def test_a_run_at_a_transition_lands_in_the_right_berlin_day(
        app_context, owned_runs):
    """The consequence the bounds exist to prevent, tested directly.

    22:30 UTC on 2026-10-24 is 00:30 Berlin on the 25th -- still CEST, so the
    offset is +2. An implementation with correct bounds but a fixed-offset day
    assignment would put this run on the 24th and pass every span test.
    """
    _run(owned_runs, dt.datetime(2026, 10, 24, 22, 30), status='ok',
         summary=_counts(posts_seen=11))
    # 23:30 UTC on the 25th is 00:30 Berlin on the 26th -- CET by then, +1.
    _run(owned_runs, dt.datetime(2026, 10, 25, 23, 30), status='ok',
         summary=_counts(posts_seen=13))

    payload = activity.summary(dt.datetime(2026, 10, 27, 12), 7)
    assert _day(payload, '2026-10-25')['posts_seen'] == 11
    assert _day(payload, '2026-10-26')['posts_seen'] == 13
    assert _day(payload, '2026-10-24')['posts_seen'] is None


def test_a_window_spanning_a_transition_counts_the_repeated_hour_once(
        app_context):
    """Seven Berlin days across the autumn change are 169 hours, not 168."""
    payload = activity.summary(dt.datetime(2026, 10, 27, 12), 7)
    span = _parse(payload['to']) - _parse(payload['from'])
    assert span == dt.timedelta(hours=169)
...
def test_activity_rejects_unsupported_window(client):
    # `07` is deliberately absent: a leading zero still names seven. What is
    # rejected is a value that is not a plain count, not an unusual spelling
    # of one.
    for days in ('2', 'nonsense', '0', '-1', '7 ', ''):
        response = client.get(f'/radar/api/activity?days={days}')
        assert response.status_code == 400, days
        # The rejected input is never echoed back into the page.
        assert days.strip() not in response.get_data(as_text=True) or days == ''
...
def test_activity_needs_a_session(anon_client):
    """302, exactly. A signed-out reader is sent to sign in."""
    assert anon_client.get('/radar/api/activity?days=1').status_code == 302


def test_activity_is_open_to_every_signed_in_reader(plain_user):
    """Recorded fetch activity is not personal and is not an admin secret."""
    response = plain_user.get('/radar/api/activity?days=1')
    assert response.status_code == 200
...
def test_ops_sends_a_signed_out_reader_to_sign_in(anon_client):
    """302 and not 403, which is the whole distinction the client depends on:
    one of these is fixed by reloading and the other never will be."""
    assert anon_client.get('/radar/api/ops').status_code == 302
...
def test_ops_answers_the_admin_with_the_existing_summaries(client):
    ...
    assert set(payload) == {'generated_at', 'spend', 'sentiment', 'market_data',
                            'capture', 'board_results', 'selected_price_ops'}
    assert payload['generated_at'].endswith('Z')
...
def test_ops_makes_no_provider_requests(client, monkeypatch):
    ...
    The names patched here are the real provider-touching entry points, and
    setattr is unguarded on purpose: if one is renamed this test errors rather
    than quietly patching nothing. An earlier version guarded them with
    hasattr against two names that did not exist, so it patched nothing and
    asserted only that the endpoint answered 200.
    """
    import features.radar.llm_sentiment as llm_sentiment
...
def test_activity_payload_carries_its_own_provenance(client):
    ...
    assert set(day) == {'date', 'posts_seen', 'posts_new', 'mentions',
                        'buckets_written', 'completed_runs', 'counted_runs',
                        'incomplete_runs', 'error_runs', 'completeness'}
