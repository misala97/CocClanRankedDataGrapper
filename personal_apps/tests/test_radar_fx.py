...
def clean():
    def wipe():
        RadarFxRate.query.filter_by(
            base=TEST_BASE, quote=TEST_QUOTE).delete(
...
def seed(pairs):
    fx.record_rates(pairs, NOW, base=TEST_BASE, quote=TEST_QUOTE,
                    source='test-fx')
...
def test_record_rates_writes_one_row_per_day(clean):
    written = fx.record_rates(
        ...
        NOW, base=TEST_BASE, quote=TEST_QUOTE, source='test-fx')

    assert written == 2
    assert RadarFxRate.query.filter_by(
        base=TEST_BASE, quote=TEST_QUOTE).count() == 2
...
def test_record_rates_restates_an_existing_day(clean):
    ...
    fx.record_rates([(dt.date(2026, 9, 1), decimal.Decimal('1.1700'))],
                    NOW, base=TEST_BASE, quote=TEST_QUOTE,
                    source='test-fx')

    rows = RadarFxRate.query.filter_by(
        base=TEST_BASE, quote=TEST_QUOTE).all()
    assert len(rows) == 1
...
def test_rate_series_returns_published_days_only(clean):
    ...
    series = fx.rate_series(dt.date(2026, 9, 1), dt.date(2026, 9, 4),
                            base=TEST_BASE, quote=TEST_QUOTE)
