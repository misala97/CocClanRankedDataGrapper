...
describe('picking segments', () => {
  it('adds a second one rather than replacing the first', () => {
    /* Michi's ask: "small and micro at once". The chips were a radio group
       and every click threw the previous selection away. */
    expect(toggleSegment(['small'], 'micro')).toEqual(['small', 'micro'])
  })

  it('turns one off without disturbing the others', () => {
    expect(toggleSegment(['small', 'micro', 'large'], 'micro'))
      .toEqual(['small', 'large'])
  })
  ...
  it('lands on All when the last one is turned off', () => {
    ...
    expect(toggleSegment(['small'], 'small')).toEqual([])
  })
  ...
  it('clears everything when All is picked', () => {
    ...
    expect(toggleSegment(['small', 'large'], null)).toEqual([])
  })
