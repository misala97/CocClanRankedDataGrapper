...
:root { --bg: #F7F2FA; --card: #FFFFFF; --edge: #DACAE3; --ink: #291238; --dim: #634674;
        ...
        --scrim: rgb(23 11 32 / .95); --on-scrim: #F8F1FB; }
...
.zoom { display: none; flex: none; padding: 0; border: 0; border-radius: 8px; background: none;
        cursor: zoom-in; }
.saved .zoom, #reference .zoom { display: block; }
.zoom:focus-visible { outline: 3px solid var(--ink); outline-offset: 2px; }
.thumb { display: block; width: 64px; height: 64px; border-radius: 8px; object-fit: cover; }
.zoom:hover .thumb { box-shadow: 0 0 0 2px var(--dim); }
...
.viewer { inset: 0; width: 100%; height: 100%; max-width: none; max-height: none; margin: 0;
          padding: 16px; border: 0; background: transparent; color: var(--on-scrim);
          cursor: zoom-out; }
.viewer[open] { display: grid; place-items: center; }
.viewer::backdrop { background: var(--scrim); }
.viewer figure { margin: 0; display: grid; gap: 10px; justify-items: center; }
.viewer img { display: block; width: auto; height: auto; border-radius: 12px;
              max-width: min(calc(100vw - 32px), 1100px);
              max-height: calc(100vh - 136px); max-height: calc(100dvh - 136px); }
.viewer figcaption { font-weight: 600; text-align: center; }
.viewer .close { position: absolute; top: 12px; right: 12px; min-width: 88px; min-height: 40px;
                 padding: 0 14px; border: 1px solid currentColor; border-radius: 8px;
                 background: transparent; color: inherit; font: inherit; font-weight: 600;
                 cursor: pointer; }
.viewer .close:focus-visible { outline: 3px solid var(--on-scrim); outline-offset: 2px; }
@media (prefers-reduced-motion: no-preference) {
  .viewer[open] figure { animation: viewer-in 220ms cubic-bezier(.16, 1, .3, 1); }
  .viewer[open]::backdrop { animation: viewer-fade 220ms ease-out; }
}
@keyframes viewer-in { from { opacity: 0; transform: scale(.96); } }
...
<main>
  ...
  <section class="card" id="reference">
    <div class="head">
      <button class="zoom" type="button" aria-label="Show the reference picture bigger"><img
        class="thumb" alt="" src="raw/reference.webp"></button>
...
<dialog class="viewer" id="viewer" aria-labelledby="viewer-title">
  ...
  <figure><img alt=""><figcaption><span id="viewer-title"></span> &middot;
    <span class="file"></span></figcaption></figure>
</dialog>
<script>
...
// Tries each picture type in turn and calls found() with the first file that loads.
function findSaved(slug, found, index = 0) {
  if (index >= TYPES.length) return;
  const name = `${slug}.${TYPES[index]}`;
  const probe = new Image();
  probe.onload = () => found(name);
  probe.onerror = () => findSaved(slug, found, index + 1);
  probe.src = `raw/${name}`;
}
...
function openViewer(src, title, name) {
  viewerImage.src = src;
  viewerImage.alt = title;
  document.getElementById('viewer-title').textContent = title;
  viewer.querySelector('.file').textContent = name;
  viewer.showModal();
}

// any click closes it, on the picture, beside it or on Close; Esc too
...
for (const item of ITEMS) {
  ...
  zoom.type = 'button';
  zoom.className = 'zoom';
  zoom.setAttribute('aria-label', 'Show ' + item.move + ' bigger');
  ...
  const verb = document.createTextNode(' \\u00b7 save as ');
  shows.append('Shows ' + item.shows, verb, file);
  title.append(h2, shows);
  const state = document.createElement('span');
  state.className = 'state';
  state.textContent = 'to do';
  head.append(zoom, title, state);
  card.append(head, row(item.brief, 'Copy prompt', 'prompt', item.prompt));
  list.append(card);
  zoom.addEventListener('click', () =>
    openViewer(thumb.src, item.n + ' ' + item.move, file.textContent));
  findSaved(item.slug, (name) => {
    thumb.src = `raw/${name}`;
    verb.textContent = ' \\u00b7 saved as ';
...
reference.querySelector('.zoom').addEventListener('click', () =>
  openViewer(reference.querySelector('.thumb').src, 'Reference picture', 'reference.webp'));
