...
- **Structure travels one way only.** The follower's structural changes never propagate back to the leader. This does *not* mean the follower is blocked from editing their own session — nothing in this plan enforces that, and the spec's "the follower's session is otherwise fully theirs" says they should not be. Reconciliation silently reverts whatever it can see; where the two collide (both partners substituting the same exercise), the follower's own choice wins for their own session.
