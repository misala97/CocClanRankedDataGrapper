...
- **The nested Week History repeats the main table and re-buries the score.** It's a second wide per-week table (the same columns) whose two charts plot avg attack/defense *stars*, not the comparable `score_100` — the very number this redesign elevates. History should read the score trajectory + the player's verdict track record, not replicate the roster row N times (see §4.5).
...
2. **Defenses** — a one-line **defense summary** (digested: defenses-held count · average stars conceded · worst hold = the opponent who took the most stars, with %) above the raw per-defense table (opponent, their TH, stars, %). Secondary on purpose (the score is attack-only). Summary derived client-side from `defense_details`/`def_*` fields — no backend. Present only when defenses exist.
3. **History** — the deep ranked dive (up to 52 weeks), **rethought to be score-first** (the profile page's light 10-week list links back here). It must *not* replicate the main table N times or re-bury the score as a verdict suffix — the score trajectory and the player's verdict track-record lead. Arrangement (chosen: "balanced dashboard"), top to bottom:
   - **Track-record summary** (figure cards): career **verdict spread** (count of weeks per verdict tier — Godlike / Dominant / Very Good / Good / Bad / Disaster), **score range** (best / typical / worst), **consistency of the score** (σ), **full-attendance** (full-attack weeks / total), plus a one-line **trend read** (improving / steady / declining). Supporting figures: peak trophies, best rank.
   - **Two charts, side by side:** primary = **`score_100` trajectory** across the weeks (0–100, verdict-tier context, latest week emphasized) — the honest "getting better or worse" line, replacing today's avg-attack/defense-stars chart; secondary = **league progression** (the strata idiom — climbing corresponds to real visited league tiers).
   - **Full per-week table, led by Score·Verdict:** every week row leads with its score + verdict badge, then week / league / rank / trophies / attacks (Ø · used/max · split) / avg-attack-TH / defenses (Ø · n · split) / avg-def-TH — keeping the week-over-week trend arrows and the NOW marker on the latest week. Complete, not thinned.
   - **Data:** verdict spread, score range, score σ, and the trend read are all derived client-side from `player_history[tag]` (each entry already carries `score_100`, `badge_class`, and the att/def stats) — no backend change. Present only when history exists.
...
5. **Tabbed drawer** — active-tab treatment (no side-stripe); the Verdict tab's formula + lever figures + the one-disclosure per-attack table; the Defenses table; the History tab's score-trajectory chart + league-progression chart, the track-record figure cards, and the verdict-spread bar — chart styling as design material (fills, grid, emphasized endpoints, verdict-tier context on the score line), reduced-motion safe.
